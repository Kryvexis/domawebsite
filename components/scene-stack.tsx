...
export function SceneStack() {
  ...
  const active = useMemo(() => scenes.find((scene) => scene.id === activeId) ?? scenes[0], [activeId]);

  return (
    <section className="section-shell py-24 md:py-28">
      <div className="grid gap-8 lg:grid-cols-[1.04fr_0.96fr] lg:items-start">
        <div>
          <div className="text-sm uppercase tracking-[0.35em] text-orange-300">Scene stack</div>
          <h2 className="mt-3 text-4xl font-black uppercase leading-[0.94] md:text-6xl">Big layered scenes that feel expensive on every page.</h2>
          <p className="mt-5 max-w-2xl text-lg leading-relaxed text-zinc-300">
            This block is built for presenting the concept. Click through the scenes to show different directions, then let the cards and detail panel animate between states.
          </p>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {scenes.map((scene, index) => (
              <motion.button
                ...
                className={`relative overflow-hidden rounded-[1.8rem] border p-5 text-left transition ${activeId === scene.id ? 'border-orange-500/40 bg-orange-500/10' : 'border-white/10 bg-white/[0.04]'}`}
              >
                <div className="text-[11px] uppercase tracking-[0.3em] text-orange-300">{scene.tag}</div>
                <div className="mt-4 text-2xl font-black uppercase leading-[0.96]">{scene.title}</div>
                <div className="absolute bottom-3 right-4 text-6xl font-black text-white/5">0{index + 1}</div>
              </motion.button>
            ))}
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-[11px] uppercase tracking-[0.34em] text-orange-300">Active direction</div>
              <div className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[11px] uppercase tracking-[0.24em] text-zinc-300">Demo interaction</div>
            </div>
            <div className="mt-5 text-3xl font-black uppercase leading-[0.95] md:text-5xl">{active.title}</div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-4">
              {[
                [Layers3, 'Layered'],
                [MousePointerClick, 'Clickable'],
                [Smartphone, 'Mobile'],
                [Sparkles, 'Polished'],
              ].map(([Icon, label], index) => {
                const Comp = Icon as typeof Layers3;
                return (
                  <div key={index} className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4">
