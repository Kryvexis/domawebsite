    ...
    <section ref={ref} className="relative py-24 md:py-32">
      <div className="section-shell grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <div className="lg:sticky lg:top-28">
          <motion.div style={{ y: titleY, scale: titleScale }}>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/products" className="inline-flex items-center gap-2 rounded-2xl bg-orange-500 px-5 py-4 font-semibold text-zinc-950 transition hover:bg-orange-400">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              {[
                ['Pinned feel', Layers3],
                ['Lux motion', Sparkles],
                ['High trust', ShieldCheck],
              ].map(([label, Icon], idx) => {
                const Comp = Icon as typeof Layers3;
                return (
                  <div key={idx} className="steel-panel rounded-2xl px-4 py-4">
        ...
        <div className="relative min-h-[720px] sm:min-h-[820px]">
          <motion.div style={{ y: cardThreeY }} className="steel-panel absolute inset-x-0 top-0 rounded-[2rem] p-5 sm:p-7">
            <div className="grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-[1.6rem] border border-white/10 bg-zinc-950/70 p-6">
                ...
                <p className="mt-4 max-w-lg text-zinc-300">Large typography and long horizontal surfaces make the composition feel premium before any button is clicked.</p>
              </div>
              <div className="rounded-[1.6rem] border border-orange-500/20 bg-orange-500/12 p-6 flex items-end">
                <div>
          ...
          <motion.div style={{ y: cardTwoY }} className="steel-panel absolute inset-x-3 top-[18%] rounded-[2rem] p-5 sm:inset-x-8 sm:p-7">
            <div className="grid gap-4 md:grid-cols-3">
              {scenes.map((scene, index) => (
                <div key={scene.title} className="rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-5 md:min-h-[280px]">
          ...
          <motion.div style={{ y: cardOneY }} className="absolute inset-x-6 top-[50%] rounded-[2rem] border border-orange-500/20 bg-gradient-to-r from-orange-500/12 via-zinc-950/90 to-white/[0.05] p-6 shadow-[0_25px_80px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:inset-x-14 sm:p-8">
            <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
              <div>
                <div className="text-[11px] uppercase tracking-[0.34em] text-orange-300">Cinematic closer</div>
                <div className="mt-4 text-3xl font-black uppercase leading-[0.95] md:text-5xl">This is the section that makes the site feel custom-built.</div>
                <p className="mt-4 max-w-xl text-zinc-300">The layered motion is subtle enough to stay professional but strong enough to clearly say this is not a cheap template.</p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                {['Parallax cards', 'Responsive scene stack', 'Better scroll rhythm', 'Presentation-ready polish'].map((item) => (
                  <div key={item} className="rounded-[1.4rem] border border-white/10 bg-zinc-950/70 px-5 py-5 text-sm uppercase tracking-[0.18em] text-zinc-100">
                    {item}
                  </div>
                ))}
              </div>
