        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_2fr]">
          <Reveal>
            <div className="steel-panel sticky top-28 rounded-[2rem] p-6">
              <div className="text-lg font-bold">Quick Filters</div>
              <div className="mt-5 grid gap-3">
                {['Power Tools', 'Plumbing', 'Paint', 'Electrical', 'Safety', 'Garden'].map((item) => (
                  <button key={item} className="rounded-2xl border border-white/10 bg-zinc-900/80 px-4 py-3 text-left transition hover:border-orange-500/30 hover:bg-white/5">
                    {item}
                  </button>
                ))}
              </div>
              <div className="mt-6 rounded-2xl border border-orange-500/20 bg-orange-500/10 p-5">
                ...
                <div className="mt-2 text-2xl font-black">Weekend Contractor Deals</div>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {featuredProducts.map((product, index) => (
              <Reveal key={product.title}>
                <div className="relative min-h-[300px] overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03))] p-6 transition hover:-translate-y-1.5">
                  ...
                  <div className="mt-6 text-2xl font-bold">{product.title}</div>
